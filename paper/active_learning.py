...
import gpytorch
import matplotlib
# use non-interactive backend for servers without a display
matplotlib.use('Agg')
...
for d in range(NUM_RUNS):
    ...
    # collect per-step metrics for this run
    metric_records = []
    while len(chosen_triplets)<df_triplet_means.shape[0]:
        ...
        try:
            ...
            t1 = time.time()
            print(f"MLP training (run {d}, iter {i}) time: {t1-t0:.2f}s")
        ...
        # compute per-step NLL and MAE for the target index
        y_true = y_test[:, col_i]
        mu_t = mu[:, col_i]
        std_t = std[:, col_i]
        # numerical floor for std
        eps = 1e-6
        var = (std_t**2).clamp(min=eps)
        # gaussian negative log likelihood per sample
        nlls = 0.5 * ((y_true - mu_t)**2 / var + torch.log(2 * torch.tensor(np.pi) * var))
        nll_step = float(nlls.mean().item())
        mae_step = float(torch.mean(torch.abs(y_true - mu_t)).item())
        metric_records.append({'run': int(d), 'step': int(i), 'nll': nll_step, 'mae': mae_step})
    ...
    # save per-step metrics for this run
    metrics_df = pd.DataFrame(metric_records)
    metrics_path = DNAME / 'metrics_stepwise.csv'
    metrics_df.to_csv(metrics_path, index=False)

    # compute per-run summary: mean NLL and MAE
    run_mean_nll = metrics_df['nll'].mean() if not metrics_df.empty else np.nan
    run_mean_mae = metrics_df['mae'].mean() if not metrics_df.empty else np.nan
    # compute steps needed to reach global max FE (index adjusted by initial picks)
    df_triplets_mean = df.groupby('triplet').mean()
    cum = df_triplets_mean.loc[chosen_triplets, 'FE (Eth)'].cummax().values
    target = df_triplet_means[col_n].max()
    inds = np.where(cum >= target - 1e-12)[0]
    if inds.size>0:
        idx_at_max = int(inds[0])
        steps_to_max = max(0, idx_at_max - (len(chosen_triplets[:3]) - 1))
    else:
        steps_to_max = np.nan

    # append per-run summary to top-level model metrics file
    model_metrics_file = Path('./MLP_F') / 'metrics_all_runs.csv'
    run_summary = pd.DataFrame([{'run': int(d), 'mean_nll': run_mean_nll, 'mean_mae': run_mean_mae, 'steps_to_max': steps_to_max}])
    if model_metrics_file.exists():
        run_summary.to_csv(model_metrics_file, mode='a', header=False, index=False)
    else:
        run_summary.to_csv(model_metrics_file, index=False)

    # compute AF across runs so far and print
    all_runs = pd.read_csv(model_metrics_file)
    EN = all_runs['steps_to_max'].dropna().mean()
    AF = 13.0 / EN if EN>0 and not np.isnan(EN) else np.nan
    print(f"MLP run {d} summary: mean NLL={run_mean_nll:.5f}, mean MAE={run_mean_mae:.5f}, steps_to_max={steps_to_max}, AF={AF}")
...
for d in ds:
    ...
    while len(chosen_triplets)<df_triplet_means.shape[0]:
        ...
        try:
            ...
            t1 = time.time()
            print(f"Ph ensemble training (run {d}, iter {i}) time: {t1-t0:.2f}s")
        except:
            print('')
            break
        ...
        X_test, y_test, _, _, test_df = normalize_df_torch(withheld_triplets, means, stds)
        mu, std = predict(X_test)
        # compute NLL and MAE for this step
        y_true = y_test[:, col_i]
        mu_t = mu[:, col_i]
        std_t = std[:, col_i]
        eps = 1e-6
    ...
    model_metrics_file = Path('./Ph_F') / 'metrics_all_runs.csv'
    run_summary = pd.DataFrame([{'run': int(d), 'mean_nll': run_mean_nll, 'mean_mae': run_mean_mae, 'steps_to_max': steps_to_max}])
    if model_metrics_file.exists():
        run_summary.to_csv(model_metrics_file, mode='a', header=False, index=False)
    else:
    ...
    EN = all_runs['steps_to_max'].dropna().mean()
    AF = 13.0 / EN if EN>0 and not np.isnan(EN) else np.nan
    print(f"Ph run {d} summary: mean NLL={run_mean_nll:.5f}, mean MAE={run_mean_mae:.5f}, steps_to_max={steps_to_max}, AF={AF}")
...
for d in ds:
    ...
    while len(chosen_triplets)<df_triplet_means.shape[0]:
        ...
        try:
            ...
            t1 = time.time()
            print(f"GP training (run {d}, iter {i}) time: {t1-t0:.2f}s")
        except:
            print('')
            break
        ...
        mae_step = float(torch.mean(torch.abs(y_true - mu_t)).item())
        # record
        if 'metric_records' not in locals():
            metric_records = []
        metric_records.append({'run': int(d), 'step': int(i), 'nll': nll_step, 'mae': mae_step})
    ...
    pd.DataFrame({'chosen_triplets': chosen_triplets, 'expected_improvements':expected_improvements}).to_csv(DNAME/'chosen_triplets.csv')
    # save metrics
    if 'metric_records' in locals():
        metrics_df = pd.DataFrame(metric_records)
        (DNAME / 'metrics_stepwise.csv').to_csv(DNAME / 'metrics_stepwise.csv', index=False) if False else metrics_df.to_csv(DNAME / 'metrics_stepwise.csv', index=False)
        run_mean_nll = metrics_df['nll'].mean() if not metrics_df.empty else np.nan
        run_mean_mae = metrics_df['mae'].mean() if not metrics_df.empty else np.nan
    else:
        run_mean_nll = np.nan
    ...
    model_metrics_file = Path('./GP_F') / 'metrics_all_runs.csv'
    run_summary = pd.DataFrame([{'run': int(d), 'mean_nll': run_mean_nll, 'mean_mae': run_mean_mae, 'steps_to_max': steps_to_max}])
    if model_metrics_file.exists():
        run_summary.to_csv(model_metrics_file, mode='a', header=False, index=False)
    else:
    ...
    EN = all_runs['steps_to_max'].dropna().mean()
    AF = 13.0 / EN if EN>0 and not np.isnan(EN) else np.nan
    print(f"GP run {d} summary: mean NLL={run_mean_nll:.5f}, mean MAE={run_mean_mae:.5f}, steps_to_max={steps_to_max}, AF={AF}")
...
for d in ds:
    ...
    while len(chosen_triplets)<df_triplet_means.shape[0]:
        ...
        try:
            ...
            t1 = time.time()
            print(f"GP+Ph training (run {d}, iter {i}) time: {t1-t0:.2f}s")
        except:
            print('')
            break
    ...
    if 'metric_records' in locals():
        metrics_df = pd.DataFrame(metric_records)
        metrics_df.to_csv(DNAME / 'metrics_stepwise.csv', index=False)
        run_mean_nll = metrics_df['nll'].mean() if not metrics_df.empty else np.nan
        run_mean_mae = metrics_df['mae'].mean() if not metrics_df.empty else np.nan
    else:
        run_mean_nll = np.nan
    ...
    model_metrics_file = Path('./GP_Ph_F') / 'metrics_all_runs.csv'
    run_summary = pd.DataFrame([{'run': int(d), 'mean_nll': run_mean_nll, 'mean_mae': run_mean_mae, 'steps_to_max': steps_to_max}])
    if model_metrics_file.exists():
        run_summary.to_csv(model_metrics_file, mode='a', header=False, index=False)
    else:
    ...
    EN = all_runs['steps_to_max'].dropna().mean()
    AF = 13.0 / EN if EN>0 and not np.isnan(EN) else np.nan
    print(f"GP+Ph run {d} summary: mean NLL={run_mean_nll:.5f}, mean MAE={run_mean_mae:.5f}, steps_to_max={steps_to_max}, AF={AF}")
...
all_df = pd.concat(all_df, axis=0)
fig.tight_layout()
# save figure to PNG (headless)
fig.savefig('cummax_FE.png', bbox_inches='tight')
plt.close(fig)
